...
class WizardMncAccountDescriptionReport(models.TransientModel):
    _name = 'wizard.account.description.report'
    ...
    def button_print_excel(self):
        ...
        filename = str(self.company_id.name) + " AR - Statement of Account with Description"

        worksheet1.merge_range('A1:D1', self.company_id.name, left_title)
        worksheet1.merge_range('A2:D2', 'STATEMENT OF ACCOUNT FOR CUSTOMER REPORT', left_title_sub)
        i = 3
        ...
        worksheet1.merge_range(i, 3, i, 4, 'INVOICE NUMBER', header_table)
        worksheet1.write(i, 5, 'INVOICE DATE', header_table)
        worksheet1.write(i, 6, 'GL DATE', header_table)
        worksheet1.write(i, 7, 'REFF MUTASI', header_table)
        worksheet1.write(i, 8, 'DESCRIPTION', header_table)
        worksheet1.write(i, 9, 'BRAND', header_table)
        worksheet1.write(i, 10, 'STASIUN', header_table)
        worksheet1.write(i, 11, 'ADVERTISER', header_table)
        worksheet1.write(i, 12, 'INVOICE AMOUNT', header_table)
        worksheet1.write(i, 13, 'UMUR PIUTANG', header_table)
        worksheet1.write(i, 14, 'RECEIPT/CN-DN NUMBER', header_table)
        worksheet1.write(i, 15, 'NOTE', header_table)
        worksheet1.write(i, 16, 'RECEIPT/CN-DN DATE', header_table)
        worksheet1.write(i, 17, 'APPLY DATE', header_table)
        worksheet1.write(i, 18, 'RECEIPT DUE DATE', header_table)
        worksheet1.write(i, 19, 'APPLIED AMOUNT', header_table)
        worksheet1.write(i, 20, 'OUTSTANDING AMOUNT', header_table)
        i += 1
        ...
        return {
            ...
            'url': 'web/content/?model=wizard.account.description.report&field=file&download=true&id=%s&filename=%s.xlsx' % (
                self.id, filename),
            'target': 'new',
        }
