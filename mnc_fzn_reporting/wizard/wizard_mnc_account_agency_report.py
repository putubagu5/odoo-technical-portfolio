...
class WizardMncAccountAgencyReport(models.TransientModel):
    _name = 'wizard.account.agency.report'
    ...
    def button_print_excel(self):
        ...
        filename = str(self.company_id.name) + " AR - Statement of Account by Agency"

        worksheet1.merge_range('A1:D1', self.company_id.name, left_title)
        worksheet1.merge_range('A2:D2', 'STATEMENT OF ACCOUNT BY AGENCY REPORT', left_title_sub)
        i = 3
        ...
        worksheet1.write(i, 14, 'NOMOR BUKTI POTONG', header_table)
        worksheet1.write(i, 15, 'RECEIPT/CN-DN DATE', header_table)
        worksheet1.write(i, 16, 'APPLY DATE', header_table)
        worksheet1.write(i, 17, 'CLEARED DATE', header_table)
        worksheet1.write(i, 18, 'RECEIPT DUE DATE', header_table)
        worksheet1.write(i, 19, 'INVOICE AFTER PPH AMOUNT', header_table)
        worksheet1.write(i, 20, 'PPH AMOUNT', header_table)
        worksheet1.write(i, 21, 'PPN AMOUNT', header_table)
        worksheet1.write(i, 22, 'TOTAL INVOICE AMOUNT', header_table)
        worksheet1.write(i, 23, 'APPLIED AMOUNT', header_table)
        worksheet1.write(i, 24, 'OUTSTANDING AMOUNT', header_table)
        ...
        return {
            ...
            'url': 'web/content/?model=wizard.account.agency.report&field=file&download=true&id=%s&filename=%s.xlsx' % (
            self.id, filename),
            'target': 'new',
        }
