...
class WizardSubLedgerReport(models.TransientModel):
    _name = 'wizard.mnc.subledger.report'
    ...
    def button_print_excel(self):
        ...
        filename = str(self.company_id.name) + " - Sub Ledger Detail"
        ...
        return {
            ...
            'url': 'web/content/?model=wizard.mnc.subledger.report&field=file&download=true&id=%s&filename=%s.xlsx' % (
                self.id, filename),
            'target': 'new',
        }
