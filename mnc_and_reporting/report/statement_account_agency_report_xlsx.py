...
class StatementAccountAgencyReportXLSX(models.AbstractModel):
    _name = 'report.mnc_and_reporting.statement_account_agency_report_xlsx'
    ...
    def set_header_data(self, arguments):
        ...
        sheet.write(header_row, 0,
                    'STATEMENT OF ACCOUNT BY AGENCY REPORT', style['title_style_align_left'])
        header_row += 1
    ...
    def set_table_header_data(self, arguments):
        ...
        headers = [
            'INVOICE NUMBER', 'ORIGINAL INVOICE NUMBER', 'INVOICE DATE', 'GL DATE', 'INVOICE DUE DATE',
            'DESCRIPTION', 'PO NUMBER', 'MO NUMBER', 'STASIUN', 'BRAND', 'ADVERTISER', 'RECEIPT/CN-DN NUMBER',
            'Nomor Bukti Potong', 'RECEIPT/CN-DN DATE', 'APPLY DATE', 'CLEARED DATE', 'RECEIPT DUE DATE',
            'INVOICE AFTER PPH AMOUNT', 'PPH AMOUNT', 'PPN AMOUNT', 'TOTAL INVOICE AMOUNT', 'APPLIED AMOUNT',
            'OUTSTANDING AMOUNT', 'UMUR PIUTANG',
        ]
    ...
    def set_table_body_data(self, arguments):
        ...
        for data in applied_invoices_data:
            ...
            write_summary_per_invoice_date = False
            move_date_month_year = False
            if data.get('move_date', False):
                move_date_month_year = data['move_date'].strftime('%b-%Y').upper()

            if not looped_date_month and move_date_month_year:
                looped_date_month.append(move_date_month_year)
            elif looped_date_month and move_date_month_year \
                    and move_date_month_year in looped_date_month \
                    and data_index and (len(applied_invoices_data) - 1) == data_index:
                write_summary_per_invoice_date = True
                last_data = True
            elif looped_date_month and move_date_month_year \
                    and move_date_month_year not in looped_date_month \
                    and data_index and (len(applied_invoices_data) - 1) == data_index:
                looped_date_month.append(move_date_month_year)
                write_summary_per_invoice_date = True
                last_data = True
            ...
            data_col = 0
            if write_summary_per_invoice_date:
                move_date_month_year_to_write = looped_date_month[-1]
                if date_index and not last_data:
                    move_date_month_year_to_write = looped_date_month[date_index - 1]
                ...
                sheet.write(data_row, data_col, \
                            'Total Bulan %s' % move_date_month_year_to_write, style['table_bold_align_left'])
                data_col += 1

                empty_count = 17
                empty_index = 0
                while empty_index <= empty_count:
                    sheet.write(data_row, data_col, '', style['table_normal_align_left'])
                    data_col += 1
                    empty_index += 1

                if total_invoice_amount:
                    sheet.write(data_row, data_col, total_invoice_amount, style['table_num'])
                else:
                    sheet.write(data_row, data_col, 0, style['table_normal_align_right'])
                data_col += 1

                if total_applied_amount:
                    sheet.write(data_row, data_col, total_applied_amount, style['table_num'])
                else:
                    sheet.write(data_row, data_col, 0, style['table_normal_align_right'])
                data_col += 1

                if total_amount_residual:
                    sheet.write(data_row, data_col, total_amount_residual, style['table_num'])
                else:
                    sheet.write(data_row, data_col, 0, style['table_normal_align_right'])
                data_col += 1
            ...
            if write_move_data:
                sheet.write(data_row, data_col, data.get('move_payment_reference', ''),
            ...
            if write_move_data:
                sheet.write(data_row, data_col, data.get('payment_term_name', ''), style['table_normal_align_left'])
            else:
                sheet.write(data_row, data_col, '', style['table_normal_align_left'])
            data_col += 1
            ...
            if write_move_data:
                sheet.write(data_row, data_col, data.get('move_po_numbers_gen21', ''), style['table_normal_align_left'])
            else:
                sheet.write(data_row, data_col, '', style['table_normal_align_left'])
            data_col += 1

            if write_move_data:
                sheet.write(data_row, data_col, data.get('move_mo_numbers_gen21', ''), style['table_normal_align_left'])
            else:
                sheet.write(data_row, data_col, '', style['table_normal_align_left'])
            data_col += 1
            ...
            sheet.write(data_row, data_col, '', style['table_normal_align_left'])  # nomor_bukti_potong
            data_col += 1
            ...
            sheet.write(data_row, data_col, '', style['table_normal_align_left'])  # invoice_after_pph_amount
            data_col += 1

            sheet.write(data_row, data_col, '', style['table_normal_align_left'])  # pph_amount
            data_col += 1

            sheet.write(data_row, data_col, '', style['table_normal_align_left'])  # ppn_amount
            data_col += 1
