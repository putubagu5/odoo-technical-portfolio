...
class PurchaseOrderListReportXLSX(models.AbstractModel):
    _name = 'report.mnc_and_reporting.purchase_order_list_report_xlsx'
    ...
    def set_header_data(self, arguments):
        ...
        sheet.write(header_row, 0, \
                    '%s PO - List Purchase Order' % wizard.company_id.name, style['header_style_align_left'])
        header_row += 1
    ...
    def set_table_header_data(self, arguments):
        ...
        headers = [
            'No', 'No PO', 'Tanggal PO', 'Supplier', 'No PR', 'Tipe PR', 'Tanggal PR', 'IDR',
            'Lokasi', 'Term of Payment', 'NOV', 'Additional Info', 'Keterangan'
        ]
