...
class AgingPDCDetailReportWizard(models.TransientModel):
    ...
    def generate_report_xlsx(self):
        ...
        if self.report_type and self.report_type == 'aging_pdc_detail':
            return self.env.ref('mnc_and_reporting.action_aging_pdc_detail_pdf'). \
                report_action(self)
    ...
    def get_receipt_per_customer(self, company_id):
        for wizard in self:
            ...
            if wizard.customer_type and wizard.customer_type == 'specific':
                domain += [('misc_partner_id', 'in', wizard.customer_ids.ids)]
