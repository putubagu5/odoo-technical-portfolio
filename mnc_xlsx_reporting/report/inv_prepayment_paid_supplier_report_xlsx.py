...
class InvoicePrepaymentPaidSupplierXLSX(models.AbstractModel):
    _name = 'report.mnc_xlsx_reporting.inv_prepayment_paid_supplier_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, wizard):
        ...
        for company in company_ids:
            sheet = workbook.add_worksheet( \
                '%s AP - Invoice Prepayment Paid For Supplier v3' % company.name)
            arguments = {
                'workbook': workbook,
                'sheet': sheet,
                'wizard': wizard,
    ...
    def set_header_data(self, arguments):
        ...
        sheet.write(header_row, 0, company_id.name, style['header_style_align_left'])
        header_row += 1
