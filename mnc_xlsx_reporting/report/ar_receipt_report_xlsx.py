...
class ARReceiptReportXLSX(models.AbstractModel):
    _name = 'report.mnc_xlsx_reporting.ar_receipt_report_xlsx'
    ...
    def set_table_body_data(self, arguments):
        ...
        for customer_id, data_per_customer in ar_receipt_datas.items():
            ...
            for receipt_id, list_of_invoice in data_per_customer.items():
                receipt = self.env['miscellaneous.miscellaneous'].sudo().browse(receipt_id)
                receipt_col = 2
                if list_of_invoice and len(list_of_invoice) > 1:
                    ...
                    sheet.write(data_row, receipt_col, receipt.receipt_number, style['table_normal_align_left'])
                receipt_col += 1

                if list_of_invoice and len(list_of_invoice) > 1:
                    sheet.merge_range(data_row, receipt_col,
                        data_row + (len(list_of_invoice) - 1), receipt_col,
                        receipt.partner_bank_id.acc_number or '', style['table_normal_align_left'])
                else:
                ...
                if receipt.remaining_amount:
                    if list_of_invoice and len(list_of_invoice) > 1:
                        ...
                        sheet.write(data_row, receipt_col, receipt.remaining_amount, style['table_num'])
                else:
                    if list_of_invoice and len(list_of_invoice) > 1:
                        sheet.merge_range(data_row, receipt_col,
                            data_row + (len(list_of_invoice) - 1),
                        ...
                        sheet.write(data_row, receipt_col, \
                            receipt.remaining_amount, style['table_normal_align_right'])
    ...
    def get_ar_receipt_where_clause(self, arguments):
        ...
        where_clause = "WHERE mm.company_id = %s" % company_id.id
