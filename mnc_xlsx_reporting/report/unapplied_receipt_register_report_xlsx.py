...
class UnappliedReceiptRegisterReportXLSX(models.AbstractModel):
    _name = 'report.mnc_xlsx_reporting.unapplied_receipt_register_xlsx'
    _inherit = 'report.report_xlsx.abstract'

    def generate_xlsx_report(self, workbook, data, wizard):
        company_ids = self.env['res.company']. \
            browse(self.env.context.get('allowed_company_ids', []))
        if not company_ids:
            company_ids = self.env.user.company_id
        for company in company_ids:
            sheet = workbook.add_worksheet( \
                '%s Unapplied Receipts Register_Details' % company.name)
            arguments = {
                'workbook': workbook,
                'sheet': sheet,
                'wizard': wizard,
    ...
    def set_header_data(self, arguments):
        ...
        sheet.write(header_row, 0, \
                    '%s Unapplied Receipts Register' % company_id.name, style['header_style_align_left'])
    ...
    def get_unapplied_account(self, arguments):
        company = arguments['company_id']
        unapplied_account = self.env['account.account']. \
            search([('code', '=', '1151501')], limit=1, order='id desc')

        if not unapplied_account:
            raise ValueError(
                _('There is no 1151501 - Unapplied AR account in %s' \
                  % company.name)
            )
    ...
    def set_table_body_data(self, arguments):
        ...
        for customer_id, data_per_customer in ar_receipt_datas.items():
            ...
            if not used_customer_name or (customer_name_and_number and used_customer_name \
                                          and customer_name_and_number not in used_customer_name):
                index += 1
                if index > 1:
            ...
            for receipt in data_per_customer:
                ...
                sheet.write(data_row, receipt_col, receipt.partner_bank_id.acc_number or '', style['table_normal_align_left'])
                receipt_col += 1

                if receipt.applied_amount:
                    sheet.write(data_row, receipt_col, receipt.applied_amount, style['table_num'])
                else:
                    sheet.write(data_row, receipt_col, receipt.applied_amount, style['table_normal_align_right'])
                total_applied_amount += receipt.applied_amount
                receipt_col += 1
            ...
            used_customer_name.append(customer_name_and_number)
            cust_col += 1
            ...
            if total_applied_amount:
                sheet.write(cust_row + count_receipt_data_per_customer, \
                            cust_col, total_applied_amount, style['table_num_bold'])
            else:
                sheet.write(cust_row + count_receipt_data_per_customer, \
                            cust_col, total_applied_amount, style['table_bold_align_right'])
            cust_col += 1
            ...
            grand_total_applied_amount += total_applied_amount
        ...
        if grand_total_applied_amount:
            sheet.write(grand_total_row, grand_total_col, \
                        grand_total_applied_amount, style['num_bold'])
        else:
            sheet.write(grand_total_row, grand_total_col, \
                        grand_total_applied_amount, style['bold_align_right'])
        grand_total_col += 1
    ...
    def get_receipt_data_with_orm(self, arguments):
        wizard, company_id = arguments['wizard'], arguments['company_id']
