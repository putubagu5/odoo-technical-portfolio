...
class AccountAgedPayable(models.AbstractModel):
    ...
    amount_residual_currency = fields.Monetary(group_operator='max')
    amount_residual = fields.Monetary(group_operator='max')

    def _field_column(self, field_name, sortable=False, name=None):
        """ Method override to change char type class to text-left """
        classes = ['text-nowrap']
    ...
    def _get_column_details(self, options):
        """ inherit function to change the title of the table """
        res = super(AccountAgedPayable, self)._get_column_details(options)
        # res[1] = self._field_column('reference_number', name='Reference Number')
        # res[2] = self._field_column('description', name='Description')
        res[10] = self._field_column('expected_pay_date', name='Due Date')
        res.pop(1)  # remove index 1, currently description
        res.insert(4, self._field_column('invoice_date'))
        res.insert(5, self._field_column('accounting_date'))
        res.insert(1, self._field_column('partner_name', name='Partner Name'))
        res.pop(2)  # remove index 2, currently reference_number
        # and put in index 1 again. Good luck understanding this
        res.insert(1, self._field_column('reference_number', name='Reference Number'))
        res.insert(12, self._field_column('amount_residual_currency', name='Remaining Amount (Original)'))
        res.insert(13, self._field_column('amount_residual', name='Remaining Amount (IDR)'))
        return res

    @api.model
    def _get_sql(self):
        """ override function to add date fields """
        # NOTE: change the reference_number to use payment_reference
        # ref to use description
        # add invoice_date and accounting_date
        # expected_pay_date to use invoice_date_due
        ...
        query = ("""
            ...
            SELECT
                ...
                move.ref as ref,
                move.payment_reference as reference_number,
                account_move_line.ref as description,
                ...
                round(COALESCE(company_currency.rate, 1) / COALESCE(used_currency.rate, 1),currency_table.precision) as currency_rate,
            ...
            JOIN res_currency rc ON account_move_line.currency_id = rc.id
            ...
            GROUP BY account_move_line.id, partner.id, trust_property.id, journal.id, move.id, account.id, rc.name, move.ref,
    ...
    def _format_total_line(self, res, value_dict, options):
        """ override function to set colspan and columns value for total """
        res['name'] = _('Total')
        res['colspan'] = 9
