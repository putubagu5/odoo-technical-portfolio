...
class AccountAgedReceivable(models.AbstractModel):
    ...
    def _get_column_details(self, options):
        ...
        res = super(AccountAgedReceivable, self)._get_column_details(options)
        res[3] = self._field_column('report_date', name='Due Date')
        res.insert(3, self._field_column('invoice_date'))
        ...
        res.insert(1, self._field_column('transaction_type', name='Transaction Type'))
    ...
    def _get_sql(self):
        ...
        query = ("""
            ...
            SELECT
                ...
                move.invoice_date_due AS report_date,
                move.invoice_date AS invoice_date,
                account_move_line.expected_pay_date AS expected_pay_date,
                COALESCE(transaction_type.name, '') AS transaction_type,
            ...
            LEFT JOIN account_transaction_type transaction_type ON transaction_type.id = move.transaction_type_id
            ...
            GROUP BY account_move_line.id, partner.id, trust_property.id, journal.id, move.id, account.id, rc.name, move.ref,
                     period_table.period_index, currency_table.rate, currency_table.precision, company_currency.rate, used_currency.rate, transaction_type.name
            HAVING CASE WHEN (
                account_move_line.company_currency_id != account_move_line.currency_id
                AND (bool_and(part_debit.debit_currency_id = account_move_line.currency_id) OR COUNT(part_debit) = 0)
                AND (bool_and(part_credit.credit_currency_id = account_move_line.currency_id) OR COUNT(part_credit) = 0)
