odoo.define('one2many_mass_select_delete.form_widgets', function (require) {
    ...
    var One2manyDelete = FieldOne2Many.extend({
		...
		delete_selected_lines: function()
		{
			...
			var w_response = confirm("Delete selected records?");
			if (w_response) {
		...
		selected_lines: function()
		{
			...
			var w_response = confirm("Select Records?");
			if (w_response) {
    ...
    _getResId: function (recordId) {
       var record;
	   // find value with same id and assign, then return the res_id
	   record = this.value.data.find((x) => x.id === recordId);
    //    utils.traverse_records(this.value.data, function (r) {
	// 	console.log(r);
    //    if (r.id === recordId) {
    //         record = r;
    //    }
