...
class HrEmployee(models.Model):
    ...
    def send_to_atis_insert(self, res_id):
        ...
        _logger.info(res_id)
        sync_log_id = self.env['x.people'].sudo().create_odoo_stg(res_id)
        self.env['x.people'].sudo().create_push_to_atis(sync_log_id.id)
        self.env['x.people'].sudo().count_data_from_atis(sync_log_id.id)
        pass

    def send_to_atis_update(self, res_id):
        _logger.info('posisi di send_to_atis_update')
        _logger.info(res_id)
        sync_log_id = self.env['x.people'].sudo().write_odoo_stg(res_id)
        self.env['x.people'].sudo().write_push_to_atis(sync_log_id.id)
        self.env['x.people'].sudo().count_data_from_atis(sync_log_id.id)
