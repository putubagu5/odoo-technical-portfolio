...
class AccountAsset(models.Model):
    ...
    wilayah_id = fields.Many2one('asset.location','Wilayah')
    ...
    def _assign_period(self):
        ...
        domain = [
            ...
            ('period_id.company_id', '=', self.company_id.id),
        ]
    ...
    @api.constrains('acquisition_date', 'company_id')
    def _check_period(self):
        """ constrains function to check period before create/write """
        for rec in self:
            domain = [
                ('date_start', '<=', rec.acquisition_date),
                ('date_end', '>=', rec.acquisition_date),
                ('period_id.company_id', '=', rec.company_id.id),
            ]
            period_line = self.env['asset.period.line'].search(domain, limit=1)
            if period_line and period_line.state == 'close':
                raise ValidationError('Asset Period is Closed!')
