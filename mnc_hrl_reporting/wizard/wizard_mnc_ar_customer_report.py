...
class WizardMncArCustomerReport(models.TransientModel):
    ...
    def button_print_excel(self):
        ...
        worksheet1.set_column('G:G', 50)
        worksheet1.set_column('H:H', 10)
        worksheet1.set_column('I:I', 15)
        worksheet1.set_column('J:J', 20)
        worksheet1.set_column('K:K', 50)
        worksheet1.set_column('L:O', 30)
        worksheet1.set_column('P:P', 15)

        filename = "AR - Customer Address"

        worksheet1.merge_range('A1:D1', 'CUSTOMER ADDRESS REPORT', left_title)
        worksheet1.merge_range('A2:D2', self.company_id.name, left_title)
        ...
        worksheet1.write(i, 15, 'Status', header_table)
        i += 1
        ...
        for partner in partner_ids:
            ...
            if partner.street2:
                if address != "" and address != False and partner.street2 not in address:
                    address += ", " + str(partner.street2)
                else:
                    address = str(partner.street2)

            if partner.city:
                if address != "" and address != False and partner.city not in address:
                    address += ", " + str(partner.city)
                else:
                    address = str(partner.city)

            if partner.state_id:
                if address != "" and address != False and partner.state_id.name not in address:
                    address += ", " + str(partner.state_id.name)
                else:
                    address = str(partner.state_id.name)

            if partner.zip:
                if address != "" and address != False and str(partner.zip) not in address:
                    address += ", " + str(partner.zip)
                else:
                    address = str(partner.zip)

            if partner.country_id:
                if address != "" and address != False and partner.country_id.name not in address:
                    address += ", " + str(partner.country_id.name)
                else:
                    address = str(partner.country_id.name)

            worksheet1.write(i, 0, partner.id, left_table)
            worksheet1.write(i, 1, partner.partner_no, left_table)
            worksheet1.write(i, 2, partner.site_ids[0].name if partner.site_ids else '', left_table)
            worksheet1.write(i, 3, partner.name, left_table)
            worksheet1.write(i, 4, partner.name, left_table)
            worksheet1.write(i, 5, '', left_table)
            worksheet1.write(i, 6, address, left_table)
            ...
            worksheet1.write(i, 14, partner.site_ids[0].code if partner.site_ids else '', left_table)
            worksheet1.write(i, 15, 'Active' if partner.active else 'Non-Active', center_table)
            i += 1
