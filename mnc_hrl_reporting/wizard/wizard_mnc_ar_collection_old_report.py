...
class WizardMncArCollectionOldReport(models.TransientModel):
    _name = 'wizard.mnc.ar.collection.old.report'
    ...
    def button_print_excel(self):
        ...
        filename = str(self.company_id.name) + " AR - Collection Detail Report (old)"

        worksheet1.merge_range('A1:H1', 'Print Date : ' + datetime.strptime(today, "%Y-%m-%d %H:%M:%S").strftime(
            "%d-%b-%Y %H:%M:%S"), right_title_sub)
        worksheet1.merge_range('A2:H2', 'COLLECTION DETAIL REPORT', center_title)
        worksheet1.merge_range('A3:H3', self.company_id.name, center_title)
        if self.is_date_range:
            worksheet1.merge_range('A4:H4', 'Period : ' + datetime.strptime(str(self.start_date), "%Y-%m-%d").strftime(
                "%d-%b-%Y") + ' s/d ' + \
                                   datetime.strptime(str(self.end_date), "%Y-%m-%d").strftime("%d-%b-%Y"),
                                   center_title_sub)
        else:
            worksheet1.merge_range('A4:H4', 'As of Period : ' + dict(self._fields['month'].selection).get(
        ...
        for partner, items in grouped.items():
            ...
            worksheet1.merge_range(i, 0, i, 7, 'Customer Name : ' + partner_id.name, left_title_sub)
            i += 1
            worksheet1.write(i, 0, 'Invoice Number', header_table)
            worksheet1.write(i, 1, 'Advertiser', header_table)
            worksheet1.write(i, 2, 'Brand', header_table)
            worksheet1.write(i, 3, 'PO#', header_table)
            worksheet1.write(i, 4, 'Sales', header_table)
            worksheet1.write(i, 5, 'Invoice Amount', header_table)
            worksheet1.write(i, 6, 'Receipt Amount', header_table)
            worksheet1.write(i, 7, 'Outstanding Amount', header_table)
            i += 1

            for item in items:
                worksheet1.write(i, 0, item['invoice_number'], left_table)
                worksheet1.write(i, 1, item['advertiser_gen21'] if item['advertiser_gen21'] else '', left_table)
                worksheet1.write(i, 2, item['product_gen21'] if item['product_gen21'] else '', left_table)
                worksheet1.write(i, 3, item['po_numbers_gen21'] if item['po_numbers_gen21'] else '', left_table)
                worksheet1.write(i, 4, item['sales_person_gen21'] if item['sales_person_gen21'] else '', left_table)
                worksheet1.write(i, 5, item['amount_total'], numb_table)
                worksheet1.write(i, 6, item['amount_payment'], numb_table)
                worksheet1.write(i, 7, item['amount_outstanding'], numb_table)
                i += 1
            ...
            worksheet1.write(i, 4, 'Sub Total', left_footer)
            worksheet1.write(i, 5, sum(item['amount_total'] for item in items), numb_footer)
            worksheet1.write(i, 6, sum(item['amount_payment'] for item in items), numb_footer)
            worksheet1.write(i, 7, sum(item['amount_outstanding'] for item in items), numb_footer)
            i += 2
        ...
        return {
            ...
            'url': 'web/content/?model=wizard.mnc.ar.collection.old.report&field=file&download=true&id=%s&filename=%s.xlsx' % (
                self.id, filename),
            'target': 'new',
        }
