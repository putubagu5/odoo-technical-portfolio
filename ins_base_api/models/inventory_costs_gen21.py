...
class InventoryCostsGen21(models.Model):
    _name = "inventory.costs.gen21"

    name = fields.Char('Name')
    line_ids = fields.One2many('inventory.costs.line.gen21', 'inventory_costs_id_gen21', string='Invetory Costs Line', readonly=True)
    ...
    def create(self, vals):
        ...
        res = super(InventoryCostsGen21, self).create(vals)
        return res

    def write(self, vals):
        """ inherit function to rewrite line number """
        res = super(InventoryCostsGen21, self).write(vals)
    ...
    def button_post(self):
        ...
        if len(self.line_ids) > 0:
            for line in self.line_ids:
                ...
                if check_journal:
                    raise ValidationError(_("Can't duplicated journal entries. No Inventory Costs: "+ line['reference1']))
                ...
                if not journal_usage:
                    raise ValidationError(_("Journal does not exist or not active. No Inventory Costs: "+ line['reference1']))
                else:
                ...
                if journal_usage:
                        ...
                        for data in data_posted:
                            if (data['reference1_gl_gen21'] == line['reference1']) and (data['date'] == line['accounting_date']):
                                ...
                                if not analytic_account_id:
                                    raise ValidationError(_("Analytic Account does not exist or not active. No Inventory Costs: "+ line['reference1']))
                                else:
                                ...
                                if not operating_unit_id:
                                    raise ValidationError(_("Operating Unit does not exist or not active. No Inventory Costs: "+ line['reference1']))
                                else:
                                ...
                                if not account_id:
                                    raise ValidationError(_("Account does not exist or not active. No Inventory Costs: "+ line['reference1']))
                                else:
                    ...
                    if new_journal:
                        ...
                        if not company_id:
                            raise ValidationError(_("Company does not exist or not active. No Inventory Costs: "+ line['reference1']))
                        else:
            ...
            if len(data_posted) > 0:
                ...
                if account_move:
                    ...
                    raise ValidationError(_("Failed null data Inventory Costs"))    
        else:
            raise ValidationError(_("Failed null data Inventory Costs"))
        return True
