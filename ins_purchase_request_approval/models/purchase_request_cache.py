...
class PurchaseRequestCache(models.Model):
    _name = 'purchase.request.cache'
    _inherit = ['mail.thread.cc', 'mail.activity.mixin']
    _description = 'Purchase Request Cache'
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        res = super(PurchaseRequestCache, self).message_new(msg_dict, custom_values)
        ...
        self.env['purchase.request'].with_context(context)._process_url(url, note)
        return res

    @api.model
    def _purge_records(self):
        """ function to purge all records """
        sql = """DELETE FROM purchase_request_cache"""
