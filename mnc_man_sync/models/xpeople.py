...
class XPeople(models.Model):
    _name = 'x.people'
    _description = 'xpeople - Odoo Staging Table - prepared for ATIS - Oracle Staging'

    source_data = fields.Char(string="Source Data")
    org_id = fields.Integer(string="Org ID")
    full_name = fields.Char(string="Full Name")
    # employee_number = fields.Char(string="Employee Number")
    # person_id = fields.Many2one('hr.employee', string="Employee")
    employee_number = fields.Many2one('hr.employee', string="Employee")
    person_id = fields.Many2one('res.users', string="Users")
    #
    ...
    def create_odoo_stg(self, loc_id):
        ...
        sl_ids = self.env['hr.employee'].search([
            ('id', '=', loc_id),
        ])
        ...
        for sl in sl_ids:
            ...
            vals = {
                ...
                'org_id': sl.company_id.org_id,
                'full_name': sl.name,
                'employee_number': sl.id,
                'person_id': sl.user_id.id,
                'sync_log_id': msl_id.id
            }

            fetch_cnt += 1

            self.env['x.people'].sudo().create(vals)
    ...
    def create_push_to_atis(self, sync_log_id):
        ...
        for log_id in log_ids:
            ...
            line_ids = self.env['x.people'].search([
            ...
            for line in line_ids:
                ...
                send_cnt += 1
                sql = "insert into XPEOPLE ( SOURCE_DATA ,ORG_ID ,FULL_NAME ,EMPLOYEE_NUMBER ,PERSON_ID, ODOO_LOG_ID) values ( :source_data, :org_id, :full_name, :employee_number, :person_id, :odoo_log_id)"

                cur.execute(sql, {'source_data': line.source_data, 'org_id': line.org_id, 'full_name': line.full_name,
                                  'employee_number': line.employee_number.id, 'person_id': line.person_id.id,
    ...
    def write_odoo_stg(self, loc_id):
        ...
        for sl in sl_ids:
            ...
            xloc_ids = self.env['x.people'].sudo().search([("person_id", "=", sl.user_id.id)])
    ...
    def write_push_to_atis(self, sync_log_id):
        ...
        for log_id in log_ids:
            ...
            for line in line_ids:
                ...
                send_cnt += 1
                sql = "update XPEOPLE set SOURCE_DATA = :source_data, ORG_ID = :org_id, FULL_NAME = :full_name, EMPLOYEE_NUMBER = :employee_number, ODOO_LOG_ID = :odoo_log_id where PERSON_ID = :person_id"

                cur.execute(sql, {'source_data': line.source_data, 'org_id': line.org_id,
                                  'full_name': line.full_name, 'employee_number': line.employee_number.id,
                                  'odoo_log_id': sync_log_id, 'person_id': line.person_id.id})
    ...
    def count_data_from_atis(self, sync_log_id):
        ...
        for log_id in log_ids:
            ...
            sql = "select count(1) as total_baris from XPEOPLE where odoo_log_id = :odoo_log_id and source_data = 'O14'"
