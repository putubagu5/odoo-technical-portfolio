...
class XLocation(models.Model):
    _name = 'x.location'
    _description = 'xlocation - Odoo Staging Table - prepared for ATIS - Oracle Staging'
    ...
    def create_odoo_stg(self, loc_id):
        ...
        # Stock Locations
        sl_ids = self.env['stock.location'].search([
            ('id', '=', loc_id),
        ])
        ...
        for sl in sl_ids:
            ...
            if sl.location_id:
                location_code = sl.location_id.complete_name
            ...
            self.env['x.location'].sudo().create(vals)
    ...
    def create_push_to_atis(self, sync_log_id):
        ...
        for log_id in log_ids:
            ...
            line_ids = self.env['x.location'].search([
            ...
            for line in line_ids:
                ...
                send_cnt += 1
                sql = "insert into XLOCATION ( SOURCE_DATA ,ORG_ID ,LOCATION_CODE ,DESCRIPTION ,LOCATION_ID, ODOO_LOG_ID) values ( :source_data, :org_id, :location_code, :description, :location_id, :odoo_log_id)"

                cur.execute(sql, {'source_data': line.source_data, 'org_id': line.org_id, 'location_code': line.location_code, 'description': line.description, 'location_id': line.location_id.id, 'odoo_log_id': sync_log_id})
    ...
    def write_odoo_stg(self, loc_id):
        ...
        for sl in sl_ids:
            ...
            xloc_ids = self.env['x.location'].sudo().search([("location_id", "=", sl.id)])
    ...
    def write_push_to_atis(self, sync_log_id):
        ...
        for log_id in log_ids:
            ...
            for line in line_ids:
                ...
                send_cnt += 1
                sql = "update XLOCATION set SOURCE_DATA = :source_data, ORG_ID = :org_id, LOCATION_CODE = :location_code, DESCRIPTION = :description, ODOO_LOG_ID = :odoo_log_id where LOCATION_ID = :location_id"

                cur.execute(sql, {'source_data': line.source_data, 'org_id': line.org_id,
                                  'location_code': line.location_code or "", 'description': line.description, 'odoo_log_id': sync_log_id, 'location_id': line.location_id.id})
    ...
    def count_data_from_atis(self, sync_log_id):
        ...
        for log_id in log_ids:
            ...
            sql = "select count(1) as total_baris from xlocation where odoo_log_id = :odoo_log_id and source_data = 'O14'"
